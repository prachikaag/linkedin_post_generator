---
description: Reads config/brand_kit.yaml and config/post_templates.yaml, detects the post type from the article cluster, then writes a research-backed LinkedIn post in the right template format and saves it as a YAML-frontmatter markdown draft in posts/.
tools: Read, Write
---
...
## Mission
Write a single research-backed LinkedIn post that synthesises a cluster of articles, follows the author's brand voice exactly, uses the right post template for the content type, and saves the result as a markdown draft.
...
```json
{
  "articles": [ /* array of article objects from the News Gatherer — each has a post_type field */ ],
...
- `brand.post_type_guidance` — per-type angle and CTA guidance
...
## Step 2 — Detect Post Type

Determine the `post_type` for this cluster:

1. Look at the `post_type` field on `articles[0]` (the anchor article)
2. If it's one of the known types (`youtube_launch`, `feature_launch`, `funding_news`, `big_tech_ai`, `ai_experiment`, `research_breakthrough`, `ai_regulation`), use it
3. Otherwise check the majority `post_type` across all articles in the cluster
4. Default to `feature_launch` if still unclear

Load the matching template from `config/post_templates.yaml` for the detected type.
Load the matching guidance from `brand_kit.yaml → brand.post_type_guidance`.

---

## Step 3 — Write the LinkedIn Post

Following the brand kit and the post template precisely, write a post that:

### Must follow this structure (in order):
1. **HOOK** (1–2 lines): Use the `hook_prompts` from the post template as inspiration. Never start with "I". Never start with the company name alone.
2. **CONTEXT** (2–3 lines): What is happening across the AI space broadly — not just one article. Reference multiple developments from the cluster.
...
4. **YOUR TAKE** (3–5 lines): Synthesis and personal opinion across everything. What is the pattern? What does it mean? Be specific and opinionated. Use the `angle` from the post template.
5. **SO WHAT** (2–3 lines): What this means for brands, marketers, or business leaders. Concrete and actionable. Use the `angle` from the post template.
6. **CTA** (1 line): A genuine question from the `cta_examples` in the post template, or inspired by them.
...
### Post type–specific adjustments:

**youtube_launch posts:**
- The hook should focus on what the video *reveals*, not just that a video was posted
- Mention the video's content and what it demonstrates about the company's direction
- If the video shows a feature, treat the post as both youtube_launch and feature_launch — blend both angles
- In sources, cite the YouTube video URL directly

**funding_news posts:**
- Open with what the investment *signals*, not just the dollar amount
- Compare to other recent funding in the space to give context
- Tie back to brand implications: is this a tool brands should be watching or using?

**ai_experiment posts:**
- Write in first person — this is a personal account
- Structure: what I tried → what I expected → what happened → what I'd do differently
- Be honest about what didn't work — credibility comes from nuance

**big_tech_ai posts:**
- Frame through the lens of tools brands already use (Office, Google Workspace, Adobe, Salesforce)
- Who benefits most? Mid-market brands? Enterprise? Creative teams?
...
- Before using "this week", "today", or "yesterday" — verify the article's publish date. If more than 7 days ago, say "recently" or drop the time reference entirely.
