...
Receive the JSON array of trending objects (each has `phrase` and `source_url`).
Extract just the `phrase` values for display.
Print: `✓ Trending keywords: {first 8 phrases joined by ", "}`
