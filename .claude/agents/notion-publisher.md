...
1. Get today's date formatted as `Month DD, YYYY` (e.g. `July 8, 2026`)
2. Build the toggle title: `{today} — {article_title}`
3. Build the status callout text: `Draft · {source_count} source(s) cited · Angle: {selected_angle}`
...
Use `mcp__Notion__notion-update-page` or `mcp__Notion__notion-create-pages` — whichever the MCP exposes for appending blocks to an existing page.
